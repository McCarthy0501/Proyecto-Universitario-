...
import { ShoppingCart, Plus, Minus, Trash2, Eye, Star, GitCompare } from 'lucide-react';
...
function ProductCard({product}) {
 ...
 const { toggleItem, isComparing } = useCompare();
 ...
 const secondImage = product.gallery?.length > 0 ? product.gallery[0].image : null;
 ...
 loading="lazy"
 className={`w-full h-48 object-cover object-center transition-opacity duration-500 ${secondImage && isHovered ? 'opacity-0' : 'opacity-100'}`}
 />
 {secondImage && (
 <img
 src={secondImage}
 alt={product.product_name}
 loading="lazy"
 className={`absolute inset-0 w-full h-48 object-cover object-center transition-opacity duration-500 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
 />
 )}
 ...
 className={`absolute top-12 right-2 p-2 rounded-full transition-opacity ${
