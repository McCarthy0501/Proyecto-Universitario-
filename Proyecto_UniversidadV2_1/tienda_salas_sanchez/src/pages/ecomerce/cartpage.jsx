...
import ConfirmDialog from "../../components/complementos/ConfirmDialog";
...
function CartPage() {
 ...
 if (!couponCode.trim()) {
 toast.error('Ingresa un cupon valido');
 return;
 }
 try {
 ...
 toast.error(err.error || 'Cupon no valido');
 }
 } catch {
 ...
 message: 'Estas seguro de que deseas eliminar todos los productos del carrito? Esta accion no se puede deshacer.',
 onConfirm: () => {
 clearCart();
 ...
 message: 'Estas seguro de que deseas eliminar este producto del carrito?',
 onConfirm: () => {
 ...
 <div className="mb-4">
 <label className="block text-sm font-medium text-gray-700 mb-1">Codigo de descuento</label>
 ...
 disabled={couponApplied}
 className="px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
 >
 {couponApplied ? 'Aplicado' : 'Aplicar'}
 </button>
 </div>
 {couponApplied && (
 <p className="text-xs text-green-600 mt-1">Descuento aplicado: -${couponDiscount.toFixed(2)}</p>
 )}
 </div>
