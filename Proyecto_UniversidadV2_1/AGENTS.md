...
## Bugs conocidos (pre-existentes, no introducidos el 2026-07-04)

| Archivo | Bug |
|---------|-----|
| `productPreviewModal.jsx` | `calculateAverageRating()` lee estado antes de que `fetchReviews()` lo actualice (race condition). Usa `alert()` en vez de `toast` |
| `cajaUers.jsx` | Usa `alert()` para logout en vez de `toast` |
